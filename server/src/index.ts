...
// Rate limiting
app.use(rateLimit(60_000, 100)); // 100 requests per minute globally
const authRateLimit = rateLimit(60_000, 10); // 10 auth attempts per minute
...
app.get('/robots.txt', (_req, res) => {
  res.type('text/plain').send(
    `User-agent: *\nAllow: /\nDisallow: /api/\nDisallow: /admin/\n\nSitemap: https://alexjersey.rocks/sitemap.xml\n`
  );
});
